...
public class WeightedGraph<V> {
	...
	public void addVertex(V vertex) {
		if (weightedHashGraph.containsKey(vertex) || vertex == null){
			...
			weightedHashGraph.put(vertex, new HashMap<V,Integer> ()); 
		}
	}
	...
	public void DoDijsktra(V start, V end) {
		...
		if (!weightedHashGraph.containsKey(start)) {
			...
			Vertex vert = new Vertex(start);
			PQueue.add(vert);
			Vertex previous = vert;

			Vertex vertexEnd = null;

			while (PQueue.peek() != null) { // while there is a minimum

				Vertex vertPoll = PQueue.poll();
				for (V next : weightedHashGraph.get(vertPoll.data).keySet()) {

					if (previous.distance + getWeight(previous.data, vertPoll.data) < vertPoll.distance || vertPoll.distance < 0) {
						
						vertPoll.distance = previous.distance + getWeight(previous.data, vertPoll.data);
						vertPoll.previous = previous;
					...
					for (GraphAlgorithmObserver<V> observer : observerList) {
						observer.notifyDijkstraVertexFinished(vertPoll.data, vertPoll.distance);
					}
			...
			while (vertexEnd.previous != null) {
				ll.add(0, vert.data);
				vertexEnd = vertexEnd.previous;
			}
			...
			for(GraphAlgorithmObserver<V> observer : observerList){
				observer.notifyDijkstraIsOver(ll);
			}
